...
```bash
$ echo "Hello" > output.txt        # Truncate and write
...
1. Saving original file descriptors
2. Opening target files with appropriate flags
3. Duplicating to stdout/stderr
4. Executing command
5. Restoring original file descriptors
...
### Complex Example
```bash
$ cat data.txt | sort | uniq | grep "pattern" > results.txt 2> errors.txt
```
...
4. Filters for "pattern"
5. Writes matches to `results.txt`
6. Writes errors to `errors.txt`

## Compilation

```bash
g++ -std=c++17 main.cpp -o shell -lreadline
```
